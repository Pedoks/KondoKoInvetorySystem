...
public class ItemService : IItemService
{
    ...
    public async Task<ItemResponseDto?> UpdateAsync(string id, ItemDto dto)
    {
        var update = Builders<Item>.Update
            ...
            .Set(i => i.UnitType,         dto.UnitType)
            .Set(i => i.BaseUnit,         dto.BaseUnit)
            .Set(i => i.PreferredUnit,    dto.PreferredUnit)
            .Set(i => i.ConversionFactor, dto.ConversionFactor)
        ...
        var result = await _context.Items
            .UpdateOneAsync(i => i.Id == id, update);
    ...
    {
        if (item.ItemType == "NonConsumable") return "High";
        if (item.Quantity == 0)               return "OutOfStock";
        if (item.Quantity <= item.MinStock)   return "Low";
        if (item.Quantity <= item.MaxStock * 0.5) return "Medium";
        return "High";
    }
